/* PointPlanner — task graph data model
 * A "project" is a set of LINES. Each line is a sequence of STATIONS (tasks).
 * Lines branch and merge by sharing stations (interchanges) or by edges that
 * cross between lines. A task becomes AVAILABLE only when every prerequisite
 * (incoming edge source) is COMPLETED.
 */
...
// Line definitions (order controls legend + z-order of overlapping segments)
window.LINES = [
...
// Stations (tasks). col/row are grid coordinates. lines = which lines pass
// through (>1 => interchange). lp = label placement.
...
// Edges = "from must be completed before to can start" AND the visual segment.
// line = which line color the segment is drawn in. df = diagonal-first routing.
window.EDGES = [
  // Design
