...
| **Line filter** | Selecting a line in the legend to bring it to the foreground of whatever view is on screen. The map always **fades** what the line does not serve, since removing stations would break its routes; a board can also **hide** them, which re-counts its groups, ranks and lanes around that line alone. See ADR 0010 |
